...
.layui-layer{
    ...
    <body >
     <div class="box"> 
		<section class = "content">
		 <form id="searchForm" action="${ctx}/rest/banner/SearchProduct" method="post" >
			<ul class = "userinfo row">
				<li>
					...
					<%-- <select id="selectCategory" name="catId" class="dropdown" onchange="updateCat();">
			       	...
			       	<input name="catName" onchange="updateCat();" onclick="showTree()" />
				...
				<li><span>商品编号：</span><input style="width: 150px;" type="text" name="codeStr"></li>
				<li><span>商品名称：</span><input style="width: 150px;" type="text" name="nameStr"></li>
				<li><input style="cursor:pointer; " onclick="submitSearchProduct();" class = "public_btn bg1" type="button" name="inquery" id="inquery" value = "查询"/><input type="checkbox" id="selectall" onchange="selectAll();" ><input type="hidden" name="isAll" value="${isAll}">选择该类下所有商品</li>
			</ul>
		 </form>
     ...
     parent.$("#productList").find("tr").each(function(){
    	...
    	if(flg){
        	...
        	parent.$("#productList").append(
        			"<tr><td><input type='hidden' name='bannerProdList["+size+"].catId' value='"+catId+"' /><input type='hidden' name='bannerProdList["+size+"].prodId' value='"+productId+"' /><input type='hidden' name='productId' value='"+productId+"' /><input type='hidden' name='BannerProdList["+size+"].prod_code' value='"+productCode+"' />"+productCode+"</td><td><input type='hidden' name='bannerProdList["+size+"].prod_name' value='"+productName+"'/>"+productName+"</td><td><input type='hidden' name='bannerProdList["+size+"].catName' value='"+catName+"'/>"+catName+"</td><td><input type='hidden' name='' value='"+marketPrice+"' />"+marketPrice+"</td><td><a class='delete_img' onclick='delecte(this)'></a> </td></tr>");
        	var array = new Array();
        	parent.$("input:hidden[name$='].prodId']").each(function(){
        		array.push($(this).val());
        	});
        	parent.$("#bannerId").val("/banner/bannerinfo?productId="+array+"&catId="+catId);
    ...
    function updateCat(){
		...
		$.ajax({ 
			type: "post", 
			url: "${ctx}/rest/banner/selectCatParnet?catId="+catId, 
			async:false, // 此处必须同步
			dataType: "json",
			data:"",
    ...
    function selectAll(){
    	...
    	if($("#selectall").prop("checked")==true){
    		...
    		parent.$("#bannerId").val("/banner/bannerinfo?catId="+parent.$("#cat_Id").val());
	...
	var setting = {
		...
		/* 树形结束*/
		/* 初始化页面内容，如果选择了类型进来就把类型选择置灰，如果选择了所有类型按钮就不让用户添加产品 */
		$(function(){
