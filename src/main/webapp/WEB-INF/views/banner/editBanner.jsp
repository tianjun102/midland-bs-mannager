...
<html lang="en" class="no-js">
	...
	<div class="box">
		...
		<form id="formId" action="${ctx}/rest/banner/saveEditBanner" method="post" enctype="multipart/form-data" method="post">
		<input type="hidden" name="catId" value="${bannerProd.catId}" id="cat_Id" >
		<input type="hidden" name="id" value="${banner.id}"  >
		<input type="hidden" name="isAll" value="${bannerProd.isAll}">
		<input type="hidden" name="bannerImg1" value="${banner.bannerImg1}">
		<input type="hidden" name="bannerImg2" value="${banner.bannerImg2}">
			<ul class = "adminfo row">
				...
				<div id="img" style= "display:inline-block; float:left;">
					<div class="imgbox">
					    <div class="imgnum">
					        ...
					        <img src="<c:if test='${!empty banner.bannerImg1 and banner.bannerImg1!="" }'>${banner.bannerImg1 }</c:if>  <c:if test='${empty banner.bannerImg1 or banner.bannerImg1=="" }'>${banner.bannerImg1 }${ctx}/assets/img/btn.png</c:if>" class="img1" />
					        <img src="" class="img2" />
					    </div>
					</div>
				</div>
                ...
                <%-- <img  src="${banner.bannerImg1 }" width="120px" height="100px" alt=""><input type="file" name="file1" id="file1" onchange="previewImg(this)">
				<li><span>小图：</span><img  src="${banner.bannerImg2 }" width="120px" height="100px" alt=""><input type="file" name="file2" id="file2" onchange="previewImg(this)"></li> --%>
				<li><span>展示方式：</span>
				...
				<li><span>图片链接：</span>
				<input maxlength="200" id="bannerId" type="text" name="bannerLinkurl" value="${banner.bannerLinkurl}" > 
				</li>
				<li id="textArea" style="display: none;"><span>页面内容：</span>&nbsp;&nbsp;<textarea style="width: 92%;min-height: 350px;resize:none; outline-color: #0099e0;" name="classDescription" id="myEditor" rows="" cols=""></textarea></li>
				<li id="searchbatton" ><span>产品信息：</span><input onclick="searchProduct(1)" style="width: 100px;text-indent: 0px;" class = "public_btn bg1" type="button" name="inquery" id="inquery" value = "查询商品"/></li>
				<c:if test="${!empty categoryList or !empty category}">
					<li id="catInfo"><span>分类：</span>
						<div style="display:inline-block" id="catName">
							<c:forEach items="${categoryList}" var="category">
							<input type='text' value="${category.catName}" disabled="disabled" >
							</c:forEach>
							<input type='text' value="${category.catName}" disabled="disabled" >
							<a onclick='deleteText()' >删除</a>
						</div>
					</li>
				</c:if>
				<div id="prodInfo" class = "table-responsive m40">
					...
					<thead>
			       	...
			       	<c:forEach items="${productList}" var="product" varStatus="sta">
						<tr>
							<td><input type="hidden" name="bannerProdList[${sta.count-1}].catId" value="${product.catId}"><input type="hidden" name="bannerProdList[${sta.count-1}].prodId" value="${product.productId}"><input type="hidden" name="productId" value="${product.productId}"><input type="hidden" name="BannerProdList[${sta.count-1}].prod_code" value="${product.productCode}">${product.productCode}</td>
							<td><input type="hidden" name="bannerProdList[${sta.count-1}].prod_name" value="${product.productName}">${product.productName}</td>
							<td><input type="hidden" name="bannerProdList[${sta.count-1}].catName" value="${product.catName}">${product.catName}</td>
							<td><input type="hidden" name="" value="${product.marketPrice }">${product.marketPrice }</td>
				...
				<li><span>排序：</span><input value="${banner.sortOrder }" type="text" name="sortOrder" onfocus="InitInput.setNumber(this,3,0,0)"><span class = "_star ">*</span></li>
			</ul>
    ...
    function delecte(ths){
		layer.open({
			  ...
			  content:
			 	'<section class = "content" style = "border:none;">'+
					'<dl>'+
						...
						'<dd>'+
							'<p style = "text-align: center; margin-top: 20px;">您确定要删除改商品吗</p>'+
						'</dd>'+
					'</dl>'+
				'</section>',
			  btn:['确定','取消'],
			  yes: function(index){
		        	...
		        	$("#bannerId").val("url?productId="+array);
		        	}else{
		        		$("#bannerId").val("");
		        	}
				    layer.close(index);
    ...
    function subumintBanner(){
    	...
    	if(required){
		$.ajax({
			url : "${ctx}/rest/banner/saveEditBanner",
			type : "post",
			cache : false,
			contentType : false,
			processData : false,
